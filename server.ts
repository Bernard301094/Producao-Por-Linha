...
async function startServer() {
  ...
  const SPREADSHEET_ID = '1z8BWaxGUuUOtHRSM4FnPBr4km9ecHaCksQZw2bymVAw';
  ...
  app.post('/api/append', async (req, res) => {
    try {
      ...
      // We append to Sheet1 or whatever the first sheet is called. 
      // Using "A:I" broadly targets the columns.
  ...
  app.post('/api/update', async (req, res) => {
    try {
      ...
      // range is A{r}:I{r} where r = rowIndex + 1
  ...
  app.post('/api/delete', async (req, res) => {
    try {
      ...
      const rows = response.data.values;
      let rowIndex = -1;
      let sheetName = 'Página1'; // Assuming default or found from get
      if (rows) {
        for (let i = 0; i < rows.length; i++) {
