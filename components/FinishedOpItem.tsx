...
export const FinishedOpItem = React.memo(({ op, openEdit, setDeletingOp, setRevertingOp, onSyncRetry, availableParadas = [], onAddForgottenParada, onConvertToOp }: any) => {
  ...
  return (
    <div className={`relative bg-white rounded-2xl xl:rounded-3xl overflow-hidden mb-3 border transition-all shadow-md hover:shadow-lg ${op.syncStatus === 'error' ? 'border-red-200 ring-1 ring-red-100' : 'border-slate-200 hover:border-slate-300'}`}>
      {/* Top accent */}
        ...
        <div className="flex items-start justify-between gap-3 mb-3 xl:mb-4">
          <div className="flex flex-col flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap mb-2">
              <span className="text-[10px] xl:text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200/60 px-2 xl:px-2.5 py-0.5 xl:py-1 rounded-lg">
                OP {op.opNumber}
              </span>
              <span className="text-[10px] xl:text-xs font-semibold text-slate-500 bg-slate-100 border border-slate-200 px-2 xl:px-2.5 py-0.5 xl:py-1 rounded-lg">
              ...
              {op.isAvulsa && (
                <span className="text-[10px] xl:text-xs font-bold text-amber-700 bg-amber-50 border border-amber-200/60 px-2 xl:px-2.5 py-0.5 xl:py-1 rounded-lg">PARADA AVULSA</span>
              )}
            </div>
          ...
          {/* Time badge */}
          <div className="shrink-0 flex items-center gap-1 text-[10px] xl:text-xs font-bold text-slate-500 bg-slate-50 border border-slate-200 px-2.5 xl:px-3 py-1.5 xl:py-2 rounded-xl">
        ...
        {op.quantidade && (
          <div className="flex items-stretch gap-2 xl:gap-3 mb-3 xl:mb-4">
            <div className="flex-1 bg-emerald-50/80 border border-emerald-100 rounded-xl xl:rounded-2xl px-3 xl:px-4 py-2 xl:py-3 flex flex-col">
              <span className="text-[8px] xl:text-[9px] font-bold text-emerald-600/70 uppercase tracking-widest leading-none mb-1">Produzido</span>
              <span className="text-2xl xl:text-3xl 2xl:text-4xl font-bold text-emerald-700 tabular-nums leading-none">{parseInt(op.quantidade).toLocaleString()}</span>
              <span className="text-[9px] xl:text-[10px] font-bold text-emerald-500/60 mt-1">UN</span>
            </div>
            <div className={`flex-1 border rounded-xl xl:rounded-2xl px-3 xl:px-4 py-2 xl:py-3 flex flex-col ${op.qntReprocesso && parseInt(op.qntReprocesso) > 0 ? 'bg-amber-50/80 border-amber-100' : 'bg-slate-50 border-slate-200'}`}>
              ...
              <span className={`text-2xl xl:text-3xl 2xl:text-4xl font-bold tabular-nums leading-none ${op.qntReprocesso && parseInt(op.qntReprocesso) > 0 ? 'text-amber-700' : 'text-slate-400'}`}>
