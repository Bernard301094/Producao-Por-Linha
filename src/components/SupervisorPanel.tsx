...
import { Factory, LogOut, CalendarDays, Clock4, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2 h-10 px-3 w-full rounded-lg border-2 border-blue-200 bg-blue-50
                   text-blue-800 text-sm font-semibold hover:border-blue-400 hover:bg-blue-100
        ...
        <div className="absolute z-50 mt-2 right-0 sm:left-0 w-72 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-blue-600">
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-full text-white hover:bg-blue-500 transition-colors"
            >
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    isCurrentMonth && !isSelected && !isTodayDay && 'text-slate-700 hover:bg-blue-50',
                    isTodayDay && !isSelected && 'text-blue-600 font-black ring-1 ring-blue-300',
                    isSelected && 'bg-blue-600 text-white font-bold shadow-sm hover:bg-blue-700',
                  ].filter(Boolean).join(' ')}
                >
                  {format(day, 'd')}
                </button>
          ...
          <div className="px-3 pb-3">
            <button
              ...
              className="w-full h-8 rounded-lg border border-blue-200 text-xs font-bold text-blue-600 hover:bg-blue-50 transition-colors"
            >
              Hoje
            </button>
          </div>
      ...
      <main className="flex-1 p-4 sm:p-8 overflow-y-auto w-full max-w-6xl mx-auto">
        ...
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left text-sm text-slate-600 min-w-[700px]">
              ...
              <tbody>
                {historyReports.length === 0 ? (
                  <tr><td colSpan={8} className="p-8 text-center text-slate-400 italic">Nenhum registro encontrado para esta data/turno na nuvem.</td></tr>
                ) : (
          ...
          <div className="lg:hidden flex flex-col divide-y divide-slate-100">
            {historyReports.length === 0 ? (
              <div className="p-8 text-center text-sm text-slate-400 italic">Nenhum registro encontrado para esta data/turno na nuvem.</div>
            ) : (
              historyReports.map((rowStr, i) => {
                const [opNumber, linha, produto, litragem, quantidade, horaInicial, horaFinal] = rowStr.split('|');
                return (
