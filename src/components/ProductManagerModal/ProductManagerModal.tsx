...
export const ProductManagerModal = ({ open, onOpenChange, products, onRefresh }: ProductManagerModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="w-full max-w-[500px] rounded-[1.5rem] p-0 border-0 overflow-hidden bg-zinc-50/95 backdrop-blur-xl shadow-2xl flex flex-col sm:top-1/2 sm:-translate-y-1/2 bottom-0 top-auto translate-y-0 max-h-[92dvh]"
      >
        <button type="button" autoFocus aria-hidden="true" className="sr-only" />

        {/* Drag handle (mobile) */}
        <div className="sm:hidden flex justify-center pt-3 pb-1 shrink-0 bg-white/50">
        ...
        <div className="bg-zinc-950 px-5 py-5 relative overflow-hidden shrink-0">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/15 flex items-center justify-center text-zinc-400 hover:text-white transition-colors shrink-0"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        {/* Search & Actions Area */}
        <div className="px-5 pt-4 pb-2 bg-white/50 shrink-0">
          <div className="relative group">
            ...
            <Input
              ...
              className="pl-10 h-11 rounded-xl border border-zinc-200/80 focus-visible:border-blue-500 focus-visible:ring-4 focus-visible:ring-blue-500/10 bg-white shadow-sm text-sm font-semibold placeholder:font-medium placeholder:text-zinc-400 transition-all"
            />
            {searchTerm && (
              <button
                onClick={() => setSearchTerm('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-zinc-100 hover:bg-zinc-200 flex items-center justify-center transition-colors"
              >
        ...
        >
          {filteredProducts.length > 0 ? (
            visibleProducts.map((p) => (
              <div key={p.produto} className={cn(
                ...
                editingProduct?.produto === p.produto
                  ? 'border-blue-500 ring-4 ring-blue-500/10 bg-white shadow-md z-10 relative'
                  : confirmDelete?.produto === p.produto
                  ? 'border-red-300 ring-4 ring-red-500/10 bg-red-50 z-10 relative'
                  : 'border-zinc-200/60 bg-white hover:border-zinc-300 hover:shadow-sm shadow-sm/50'
              )}>
                {/* Product Row */}
                <div className="flex items-center gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] sm:text-sm font-black text-zinc-800 truncate leading-tight">{p.produto}</p>
                ...
                {/* Edit Form */}
                {editingProduct?.produto === p.produto && (
                  <div className="px-3 pb-3 space-y-3 border-t border-zinc-100 bg-zinc-50/50 pt-3">
                    <div className="grid grid-cols-[2fr,1fr] gap-2">
                      <div className="space-y-1">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest pl-0.5">Nome do Produto</label>
                        <Input
                          value={newName}
                          onChange={(e) => setNewName(e.target.value)}
                          className="h-9 text-sm font-bold bg-white border border-zinc-200 focus-visible:border-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500/20 rounded-lg shadow-sm"
                      ...
                      <div className="space-y-1">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest pl-0.5">Volume</label>
                        <Input
                          value={newLitragem}
                          onChange={(e) => setNewLitragem(e.target.value)}
                          className="h-9 text-xs font-bold bg-white border border-zinc-200 focus-visible:border-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500/20 rounded-lg shadow-sm"
                          placeholder="Ex: 1,5L"
                          onKeyDown={(e) => { if (e.key === 'Enter') handleSave(); if (e.key === 'Escape') setEditingProduct(null); }}
                        />
                      </div>
                    ...
                    <div className="flex justify-end gap-2 pt-1">
                      <button
                        ...
                        className="px-3 h-8 flex items-center justify-center bg-white hover:bg-zinc-100 text-zinc-600 text-xs font-bold rounded-lg border border-zinc-200 transition-colors shadow-sm"
                      >
                        Cancelar
                      </button>
                      <button
                  ...
                  <div className="px-3 pb-3 border-t border-red-100 bg-red-50/50 pt-3">
                    ...
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="px-3 h-8 flex items-center justify-center bg-white hover:bg-zinc-100 text-zinc-700 text-xs font-bold rounded-lg border border-zinc-200 transition-colors shadow-sm"
                      >
                        Cancelar
                      </button>
                      <button
            ...
            <div className="py-12 flex flex-col items-center gap-3 text-center">
              ...
              <div>
                <p className="text-[13px] font-black text-zinc-600">Nenhum produto encontrado</p>
        ...
        {/* Footer */}
        <div className="px-5 py-3 shrink-0 bg-white/50 border-t border-zinc-100">
          <button
            onClick={() => onOpenChange(false)}
            className="w-full h-10 rounded-xl bg-white hover:bg-zinc-50 border border-zinc-200 shadow-sm text-zinc-700 font-black text-xs transition-colors flex items-center justify-center"
          >
            Fechar janela
          </button>
        </div>
